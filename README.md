...
| `pr-review` | `.claude/rules/` のプロジェクト固有ルールに基づいて GitHub の Pull Request をレビューし、インラインコメントとサマリーコメントを残す |
| `commit-review` | `.claude/rules/` のプロジェクト固有ルールに基づいてローカルのコミットをレビューし、シークレット混入やコミットメッセージ規約違反などを検知する |
