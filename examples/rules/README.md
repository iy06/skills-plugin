...
- **frontmatter の `paths` を正確に書く。** Claude Code がスコープ指定として解釈するのは
  **このキー名だけ**。別名（`applies_to` 等）にすると条件が無視され、そのルールは全セッションに
  **無条件で常駐する**（実測例: 20 ファイル・約 230,000 文字 ≒ 57,500 トークンが、対象ファイルを
  一切触っていないセッションにも読み込まれていた）。リポ全体に効かせたいものだけ `["**/*"]` にする
